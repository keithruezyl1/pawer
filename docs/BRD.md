...
| D-54 | The headline shrinks until **every word fits the line whole**, capped at 24 sp over two lines | Fit on line count and height only (D-52), capped at 30 sp over three | `StaticLayout` does not overflow a word too wide for its line, it BREAKS it — a real widget rendered `No outages today` as `No / outage / s today`. Neither line count nor height catches that, because a broken word still satisfies both, so the fit loop now also refuses any size at which a single word would not fit. Measured across every headline the renderer produces, at the size the launcher actually gives the widget: 30 sp over three lines was too big, and 24 over two holds every state within 3 sp of the others with no word split anywhere. A 2x2 is unaffected either way, its height budget pinning everything to 13 sp long before the cap matters |
| D-55 | The app reads the **live feed** over a conditional GET, and a malformed payload is rejected **whole** | Keep the fixture; accept the valid entries and drop the rest | Everything the pipeline produces was invisible until now — the app read a bundled fixture, so today's parser fixes reached nobody. `FeedSource` was already shaped for this, so it is one file and a one-line swap: the ETag was already persisted and `refresh()` already handled fresh/unchanged/error with last-known-good. Verified against the live feed: 131 outages validate, a second request with the returned ETag comes back 304 with no body, 11 KB gzipped when it is not. Partial acceptance was rejected on principle — dropping entries that fail validation is a silent false negative, and an outage nobody hears about is the one failure this product cannot have, so yesterday's complete data beats today's partial data (FR-18) |
| D-56 | A failed refresh records **why**, and the dashboard shows one of three screens accordingly | One offline screen for every failure (the shipped behaviour) | `OfflineState`, `FeedErrorState` and `UpdateRequiredState` were all built and only the first was reachable, because nothing distinguished the causes. Now `offline` means we never reached the server and their saved data still stands, `server` means it answered badly and that is ours to fix, and `outdated` means the feed uses a schema this build cannot read — which makes `UpdateRequiredState` reachable without waiting on `version.json`, since the advisories file carries its own `schema_version`. Blaming a user's connection for our broken feed is the kind of small dishonesty this product is supposed to avoid |
| D-57 | The feed validator lives in **`@pawer/shared`**, not the app | Keep it beside the fetch in the app | Both ends need it and the app has no test runner, so a validator there would have shipped untested. In shared it is covered by 19 tests and the pipeline can use the same function to check its own output before publishing, which is the cheaper place to catch a bad feed |
